...
def plot_learning_curves(history: dict):
    ...
    if not history or 'epoch' not in history or not history['epoch']:
        st.warning("No training data available to plot.")
        return
...
def run_app():
    ...
    with tab_training:
        ...
        with col_cfg:
            ...
            btn_train = st.button("🚀 Start Training", type="primary", use_container_width=True, disabled=st.session_state.get("training_active", False))
        ...
        if st.session_state.get("training_active", False):
            # Re-initialize widgets for the running state
        ...
        if "train_history" in st.session_state and st.session_state.train_history.get("epoch"):
            st.divider()
